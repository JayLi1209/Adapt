...
**最终结论（到此为止，本轮"让 cem_fir 全面最优"的调查收尾）**：8 个之前
"输"的 (config,p) 点，7 个靠多种子统计（零改动）翻盘或打平；剩下 1 个
（config2 p=0.4）经过 12 种参数/门控变体 + 1 次"去掉 retrain"的替代方案
测试后，确认是当前 SFI/SFIR 固定节奏机制在这个具体 (网格几何, 变化幅度)
组合上的真实、无法通过最小改动修复的短板，且用户已经明确接受这个结果、
选择保留方法定义的完整性而非追逐这一个点的分数。
