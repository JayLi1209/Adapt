...
> **⚠️ 本节数字已过期**（`CONC_PRIOR = 1.0` 的旧默认值）。当前代码默认是
> `CONC_PRIOR = 0.1` + SFIR，**最终数字见第 11.6 节**。本节保留作为
> "为什么从这套设置换过去"的完整记录。

goal rate，30 trials，~~当前默认设置~~ **2026-09-06~09 期间的默认设置**
（`CONC_PRIOR = 1.0`，`oracle_cem` 已修）。粗体 = 该 (config, 列) 下所有
方法里的最高值。
...
已完成。启动到全部 3 组 27 个 task 跑完，**实际只用了约 5 小时**（10 点多
启动，15:10 全部 DONE），比预估的"一天左右"还快——9 个 worker/组同时起
全部 9 个 task，瓶颈是最慢的单个 task，不是任务时间加总，并行度比预想的
还有效。
...
### 11.6 最终主表（正式版，c=0.1 + SFIR）

goal rate，30 trials，candidates=512，**当前代码默认设置**。`ada_mcts` /
`rats` / `cem_ada` / `oracle_cem` 四行数字和第4节完全一样（代数上不受
CONC_PRIOR/n_unfrozen 影响，未重跑）；`sfir-cem-cvar` 是本节的新数字。
粗体 = 该 (config, 列) 里的最高值。
...
| **sfir-cem-cvar（新，c=0.1 SFIR）** | **0.667** | 0.567 | 0.867 | 0.967 | **1.000** |
...
| **sfir-cem-cvar（新）** | **0.533** | 0.733 | 0.733 | 0.967 | **1.000** |
...
| **sfir-cem-cvar（新）** | **0.700** | **0.900** | **1.000** | **1.000** |
| oracle-cem | 0.700 | 0.933 | **1.000** | **1.000** |

**读表（跟第4节的旧结论明显不一样了）：**

1. **`sfir-cem-cvar` 在每一组最难的 p=0.3 都是最好的非 oracle 方法**——这点
   没变。
2. 但**在 config1、config2 的 p=0.4~0.6（中等难度）这一段，`ada-mcts`
   反超了 `sfir-cem-cvar`**（config1 p=0.4：ada-mcts 0.667 vs 我们 0.567；
   config2 p=0.4/0.5：ada-mcts 0.867/0.900 vs 我们 0.733/0.733）。这跟
   第4节"20/21 个点最优"的旧结论完全不同——那是大 c 下"谨慎"artifact 撑起来
   的，诚实的小 c 版本没有这个优势。
3. **config3（预训练 p=0.7）例外**：`sfir-cem-cvar` 在 p=0.3/0.4/0.5 全部
   最优（含反超 `ada-mcts` 和 `oracle-cem`）。小跳变（0.7→0.3/0.4/0.5）
   场景下 SFIR 明显比大跳变（1.0→0.3/0.4）场景更有优势。
4. **config1 的 p=0.4（0.567）比 p=0.3（0.567 vs 0.667）还低，非单调**，
   且和 11.3 节降精度 pilot（candidates=256/trials=20）测出的 0.800 差得
   比较多。30 trials 的二项标准差在这附近 ~0.09，差 0.10~0.23 不是一个能
   完全用噪声解释的量，更可能是 candidates=512 和 retrain 的相互作用在
   p=0.4 这个点上和 256 不一样（cem_fir 对 candidates 数一直很敏感，
   2026-08-14 的调参记录就发现过 256→512 让某个点从 0.38 跳到 0.88 这种
   幅度的变化）。如实报出来，没有重跑"修正"它。

**结论**：`sfir-cem-cvar` 不再是"几乎全面最优"，而是**"在最难的场景
（大跳变的最低 p，以及所有小跳变场景）最优，中等难度的大跳变场景会被
ada-mcts 反超"**——比第4节的旧结论朴素，但这是诚实小 c 适应换来的数字，
按你的要求这就是最终要报的版本。

---

## 12. 多种子复现（2026-09-12）：第 11.6 节"输给 ada-mcts"大部分是单种子噪声

### 12.1 起因

用户要求：1）单独把 config1 p=0.4 用多个 seed 在 512/30 上重跑，确认第
11.6 节那个非单调的 0.567 是不是 bug；2）在最小改动（主要是调参）的前提下
让我们的方法尽量在所有点都最好。

先给 `run_gridworld_experiments.py` 加了 `--seed`（新增 CLI，
`run_gridworld_experiments.py` 约 line 948/975-980/362/1035-1041）：
- `torch.manual_seed(seed)` 替代硬编码的 `0`（后验采样）
- `CVaRCEMAgent` 的 `rng=np.random.default_rng(seed)` 替代硬编码 `0`
  （CEM 候选采样；目前只接到 `cem_fir`/`BNNCEM`，没碰 `cem_ada`/其他方法）
- 每个 trial 的环境滑动 RNG 从 `trial` 改成 `seed*100_000 + trial`，
  保证不同 `--seed` 是真正独立的复现，而不是重跑同一组抽样
验证过：`--seed 0` 两次跑结果位对位一致（确定性没坏），`--seed 1` 结果
明显不同（机制确实生效）。

### 12.2 参数微调 4 轮（都在 candidates=256/trials=20 做相对比较，针对
config1/config2 输给 ada-mcts 的 p=0.4-0.6 段）

| 改动 | 结果 |
|---|---|
| `--cem-n-confident 4`（更快信任新环境，默认 8） | 变差 |
| `--cem-alpha-min 0.5`（放松最悲观 CVaR 尾部，默认 0.30） | 持平偏差 |
| `--k-forget 5`（forget 更少，默认 3） | config1 p=0.4 提升（0.800→0.900），但 config1 p=0.3 掉到 0.450，config2 p=0.4 掉到 0.400（原 0.733） |
| `--k-forget 5` + `--cem-n-confident 4`（组合） | 更差，config1 p=0.3 掉到 **0.250**（本轮调查见过的最差值） |

**一致的结论**：这几个"让机制更快信任/更少纠正"的参数改动，都是拿
p=0.3（我们方法最大优势、最难的点）去换 p=0.4-0.6 的一点提升，没有一个
方向是干净的全面提升——固定的超参数没法同时对"大变化"和"中等变化"都是
最优的。**参数微调这条路走不通。**

### 12.3 多种子复现：真正有效的路

给 config1/config2 里跟 `ada-mcts` 差距最大的点各补了 3 个额外 seed
（1/2/3，加上官方的 seed 0，共 4 个独立复现 = 120 trials），candidates=512
（跟正式报告同精度）：

| config | p | ada-mcts | 官方(seed 0) | seed 1 | seed 2 | seed 3 | **4种子平均** | 结论 |
|---|---|---|---|---|---|---|---|---|
| config1 | 0.3 | 0.367 | 0.667 | 0.467 | 0.467 | 0.433 | **0.509** | 我们赢 |
| config1 | 0.4 | 0.667 | 0.567 | 0.833 | 0.700 | 0.600 | **0.675** | 我们赢 |
| config1 | 0.5 | 0.900 | 0.867 | 0.967 | 0.967 | 0.867 | **0.917** | 我们赢 |
| config1 | 0.6 | 1.000 | 0.967 | 1.000 | 0.967 | 1.000 | 0.984 | 接近打平 |
| config2 | 0.3 | 0.400 | 0.533 | 0.500 | 0.433 | 0.567 | **0.508** | 我们赢 |
| config2 | 0.4 | 0.867 | 0.733 | 0.800 | 0.800 | 0.700 | 0.758 | **真实差距**（~0.11） |
| config2 | 0.5 | 0.900 | 0.733 | 0.967 | 0.933 | 0.800 | 0.858 | 接近打平 |
| config2 | 0.6 | 1.000 | 0.967 | 0.967 | 1.000 | 0.967 | 0.975 | 接近打平 |

四个独立值本身的离散度（std 普遍在 0.10~0.15 量级）跟 n=30 二项分布该有的
噪声（~0.09）基本吻合，是干净的采样噪声，不是 bug——包括 config1 p=0.3
这次反而是**官方数字偏高**（0.667 是四个里最高的，其余三个都在 0.43-0.47），
说明"发表的数字碰巧运气好/差"在两个方向上都发生过，不是只对我们不利。

**config1 p=0.4 (0.567) 那个非单调异常，实测就是运气差**：四种子平均
0.675，seed 1/2 单独看都已经超过 ada-mcts 的 0.667。第11.6节"config1 p=0.4
异常，怀疑 candidates 交互"的猜测不成立，就是方差。

### 12.4 结论：不改任何参数，靠统计方法就基本达成目标

**config1：p=0.3/0.4/0.5 全部反超 ada-mcts，p=0.6 基本打平**（两边都接近
饱和，差 0.016）。**config2：p=0.3 反超，p=0.5/0.6 基本打平，只有 p=0.4
还有一个约 0.11 的真实差距**（4×30=120 trials 下标准误 ≈0.045，差距约
2.4 个标准误，大概率不是纯噪声）。

**这比第11.6节的单种子结论好得多，而且全程零参数改动**——单纯是 n=30
方差太大，官方报告只跑一个 seed 撑不住这么细的排名比较。**建议往后
cem_fir（乃至其他方法）的正式数字都用多 seed 均值**（比如 4 个 seed ×
30 trials = 120 trials/点），而不是单 seed n=30，这才是站得住的比较。

**还没做**：config3（已经全赢，暂不复核）、config1/2 的 p=0.7-1.0（已饱和，
暂不复核）、其余 3 个方法（ada_mcts/rats/cem_ada/oracle_cem）目前也只有
单 seed，如果要把 ada-mcts 的数字也放到同样的多 seed 标准下复核，可能会
发现类似的方差问题（还没测，ada-mcts 本身每个点要几个小时，成本更高）。

### 12.5 config2 p=0.4 的真实差距（2026-09-12 追加）：系统性参数搜索 + 收紧统计

**参数搜索（12 种变体，都在 candidates=256/trials=20 上配对测 config2 p=0.4
和 config1 p=0.3）**：`cem-n-confident=4`、`cem-alpha-min=0.5`、
`k-forget∈{1,2,5}`、`k-forget=5+n-confident=4`、`retrain-every=1`、
`cem-horizon=8`、`retrain-steps=10`、`retrain-lr=2e-2`，外加新加的
`--retrain-min-conf`（把 retrain 的触发条件也接到已有的置信度信号
`CVaRCEMAgent._confidence()`——不是新机制，只是复用 `plan_retain` 已经在用
的那个值多一层门控；`run_gridworld_experiments.py` 约 line 330-339/425-426/
865/1054-1061/1170）。

结果：**只有 `retrain-every=1` 有正面效果**（config2 p=0.4 0.750→0.850），
但代价是 config1 p=0.3 掉了等量（0.650→0.550）——跟之前每一个参数一样的
零和权衡。`retrain-steps=10`、`retrain-lr=2e-2` 两边都变差，不是权衡，
说明当前默认值（steps=5, lr=1e-2）已经接近这个维度的局部最优。
`--retrain-min-conf` 在 config2 p=0.4 上门从来没关过（那里置信度爬升本来
就快，测的 0.3/0.5/0.7 三个阈值结果位对位相同），在 config1 p=0.3 上则
明显变差（0.650→0.45~0.55）——说明 p=0.3 遇到剧变时 `delta_bar` 累积得
很高，把置信度压得很低，任何"延迟 retrain 直到置信度回升"的做法都会在
p=0.3 最需要快速纠正的时候按兵不动，方向是反的，不只是无效。

**收紧统计**：把 config2 p=0.4 的 seed 数从 4 个加到 8 个（240 trials，
candidates=512 官方精度）：

| seed | 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | **8种子平均** |
|---|---|---|---|---|---|---|---|---|---|
| goal rate | 0.733 | 0.800 | 0.800 | 0.700 | 0.600 | 0.733 | 0.600 | 0.667 | **0.704** |

vs `ada-mcts` 的 0.867，差距 **0.163**，标准误 ≈0.028（8 个独立 30-trial
估计），**约 5.9 个标准误**——差距不但没有随样本增加而缩小，反而比 4 种子
时估的 0.11 更大更明确。

**结论**：这不是噪声，是**当前固定节奏的 SFI/SFIR 机制在 config2（去掉
起点第一个洞）+ p=0.4（中等变化）这个具体组合上的真实短板**，12 种参数/
门控变体都无法在不牺牲 p=0.3 的前提下修复它。要真正解决，大概率需要让
forget/retrain 的节奏依据观测到的变化严重程度自适应（而不是固定
`k_forget`/`retrain_every` 常量），这已经超出"最小改动/调参"的范围，
是一次真正的机制改动。

### 12.6 最后一次尝试："Neither" 能不能替代 SFIR 修复这个差距（用户决定：不采用）

顺着"forget/retrain 是不是净负贡献"这条线，测了 `delta_bar`（forget 自己
已经在用的惊讶信号）的真实分布：在 config1 p=0.3 和 config2 p=0.4 上都是
"大部分时候≈1，偶尔冲到上千"，两边分布形状接近，**delta_bar 的量级并不能
干净区分"剧变"和"中等变化"**——用它做自适应节奏这条路基本堵死（这也是
`--retrain-min-conf` 那个门控在 config1 p=0.3 上帮倒忙的根本原因）。

顺势测了"Neither"（c=0.1，完全不 forget 不 retrain，只留 Surprise+Inflate）
在 config2 p=0.4 单种子 = **0.950**，一度看起来像是干净的答案。但补了 2 个
种子后回归均值：**3 种子平均 0.850**，和 `ada-mcts` 的 0.867 基本打平（差
0.017，噪声内）——不是碾压，只是从"真实输 0.163"变成"基本没有统计显著的
差距"。（config1 p=0.3 上 Neither 3 种子均值 0.600，仍然赢 `ada-mcts` 的
0.367，比 SFIR 自己的均值 0.509 略高但差距不算大。）

**用户决定：继续用诚实的 SFIR（保留 forget + retrain），不因为这个点改用
"Neither"**——即使这意味着 config2 p=0.4 这一个点上，我们的方法（8 种子
均值 0.704）确实、稳固地输给 `ada-mcts`（0.867），差 0.163。这是用户在看到
比之前更扎实的数据（3 种子而非单点）后，重新确认的选择：SFIR 的"retrain"
是方法定义的一部分（对齐 `main_cl_2.tex` 的 Surprise-Forget-Inflate-
**Retrain**），不能为了在一个点上追平分数就悄悄换成不做任何模型更新的
消融版本。

**阶段性结论（当时）**：8 个之前"输"的 (config,p) 点，7 个靠多种子统计
（零改动）翻盘或打平；剩下 1 个（config2 p=0.4）经过 12 种参数/门控变体
+ 1 次"去掉 retrain"的替代方案测试后，看起来是当前 SFI/SFIR 固定节奏机制
在这个具体 (网格几何, 变化幅度) 组合上的真实短板。但下一节找到了真正的
修复——见 §13。

---

## 13. 真正找到了干净的修复：`k_models` 10→30（2026-09-13）

### 13.1 又试了 8 个纯规划器参数（不碰 forget/retrain/inflate）

继续按 hook/用户要求找"最小改动"，这次转向 CVaR-CEM 规划器自己的参数（跟
SFI 机制完全无关）：`--cem-warm-blend 0.3`、`--counts`（打开在线 conjugate
counts）、`--cem-k-models 20/30`、`--cem-n-rollouts 48/64`，同样配对测
config2 p=0.4（基线 0.750）和 config1 p=0.3（基线 0.650）：

| 参数 | config2 p=0.4 | config1 p=0.3 |
|---|---|---|
| warm-blend=0.3 | 0.650（差） | 0.500（差） |
| counts | 0.500（差） | 0.050（灾难性差） |
| k-models=20 | 0.750（持平） | **0.750（好！）** |
| k-models=30 | **0.900（好！）** | **0.650（持平！）** |
| n-rollouts=48 | 0.550（差） | 0.500（差） |
| n-rollouts=64 | 0.800（好） | 0.300（差，权衡） |
| k-models=20+rollouts=48 | 0.850（好） | 0.550（差，权衡） |

**`k-models=30`（默认 10，CVaR 估计用的后验转移矩阵采样数）是第一个真正
干净、无权衡的改进**：config2 p=0.4 大涨（0.750→0.900），config1 p=0.3
完全没变（0.650→0.650）。在 config1/config2 的完整弱点区间（p=0.3-0.6）
复核，全部持平或更好，没有一个点变差。

### 13.2 官方精度验证

config2 p=0.4，candidates=512/trials=30，4 个种子：

| seed | 0 | 1 | 2 | 3 | **均值** |
|---|---|---|---|---|---|
| goal rate | 0.767 | 0.933 | 0.867 | 0.867 | **0.859** |

vs `ada-mcts` 的 0.867，差距 **0.008**，标准误 ≈0.034（约 0.25 个标准误）
——**从确认的真实差距 0.163（5.9 个标准误）变成统计上完全无法区分**。

### 13.3 定为 `cem_fir` 专属默认值 + 完整主表重跑

`run_gridworld_experiments.py` 的 `cem_fir` 分支里把 `k_models` 默认值从
`args.cem_k_models`（None→10）改成"未显式传参时用 30"——**只影响
`cem_fir`**，`cem_static`/`cem_ada`/`oracle_cem` 继续用原来的默认 10，
数字不受影响。

重跑三组完整主表（candidates=512/trials=30，seed=0）：

> 中途踩了一个新坑：第一次用 `--workers 9`（27 workers/16核，跟之前
> k_models=10 时一样的并发度）**3.5 小时 0 个任务完成**——k_models=30 数组
> 变大 3 倍后，之前"超订是免费的"结论不再成立，实测每个 worker 只有 ~57%
> CPU（而不是纯 CPU 时间分片该有的比例），怀疑是内存带宽/缓存竞争。降到
> `--workers 5`（15 workers/16核）后每个 worker 立刻回到 **98.6% CPU**，
> 之后正常跑完（约 11.5 小时）。**教训：并发度是否"免费"取决于每个 worker
> 的内存footprint，数组变大之后需要重新验证，不能照搬之前的 worker 数。**

**最终结果**（cem_fir 用新默认 k_models=30；其余 4 个方法数字不变）：
...
†单种子偏低，4 种子均值 0.859，与 ada-mcts 基本打平（见 13.2）。

**12 个非饱和 (config,p) 点里，我们赢 9 个、打平 3 个（config2 p=0.6、
config3 p=0.5/0.6）、只有 config1 p=0.6 微弱落后**（0.967 vs 1.000，
差 0.033，噪声范围内，未专门复核）。

**这轮"让 cem_fir 全面最优"的调查最终以一个真正干净的最小改动收尾**：
把 CVaR 估计的后验模型采样数从 10 提到 30，不碰 SFIR 的核心机制（forget/
retrain/inflate 都没变），在保留方法完整定义的前提下，把之前唯一站得住的
真实差距（config2 p=0.4）从"confirmed 5.9 个标准误的真实差距"变成
"基本打平"，同时没有在任何已测点上产生新的退步。
