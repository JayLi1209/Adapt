...
| cliffwalking | 0.4 | 0.567 | 0.233 | 0.667 | 0.300 |
| cliffwalking | 0.9 | 1.000 | 1.000 | 1.000 | 1.000 |
...
诊断开关 `--k-forget 1 --no-forget-reset` 可模拟该行为。**已验证（bridge，
60 trials）**：模拟旧版 → p=0.9 **0.900**（现行 1.000，cem_ada 1.000），p=0.4
0.183（现行 0.200）——旧版行为确实在 bridge 上造出 p=0.9 的差距，支持该解释。
...
| 0.6 | 0.400 | 0.400 | 0.350 | 0.350 | 0.233 | **0.217** | 0.400 | 0.467 |
| 0.8 | **0.283** | 0.300 | 0.350 | 0.383 | 0.450 | 0.483 | 0.733 | 0.767 |
...
ml-only / gate-only 在 p=0.6 各自只掉 0.05（<1 SE），叠加才掉到 0.233 —— 伤害主要来自组合。

**cliffwalking（K=10，horizon 6，30 trials）**：p=0.9 全员 1.000（无差距）。
| p | 对照 | ml+gate | cem_ada |
|---|---|---|---|
| 0.3 | **0.667** | **0.333（−0.333，约 2.7 SE）** | 0.100 |
| 0.4 | 0.567 | 0.733 | 0.233 |
| 0.5 | 0.867 | 0.967 | 0.667 |
| 0.9 | 1.000 | 1.000 | 1.000 |

**结论：ml+gate 不能当默认** —— 它在 frozenlake 的温和点有帮助，但在
cliffwalking 最剧烈的 p=0.3（正是我们主表赢最多的地方）掉 0.333。本质是
"少遗忘"与"多遗忘"的取舍：一个 episode 内的证据太少，分不清 p=0.9 和 p=0.5，
任何往"少遗忘"推的规则都会在剧烈变化时付代价。

**⚠ 顺带发现（已在交付文档加更正说明）**：`--cem-horizon` 默认值是 3，主表
用 6。09-17 的两次运行漏了这个参数：36 格饱和点补跑（进了交付文档）和 K=10
retain factor 试验 → 实际都是 horizon 3。饱和点已以 horizon 6 重跑完毕
（2026-09-23，36/36 全为 1.000），交付文档 §1/§3 已改回无警告版本。K=10 retain
试验的内部对比仍有效（对照与变体同为 h=3），但不能称为"主表设置"。

### 线5（2026-09-24 新开）：surprise 单步截断 `--surprise-cap τ`

用户决定："要。先试试截断，如果不行，再试试标准化分数。"
依据（实测，K=10，20 trials）：p=0.9 单次 slip 的 δ 比 p=0.5 大得多
（frozenlake 64.8 vs 9.4；bridge 2772 vs 1354），原因是 slip 时 H 很小
（0.71 vs 1.04；0.003 vs 0.81）；而**尖峰次数** p=0.5 反而更多 → "一直高就压低"
的滑动基线方案方向相反（离线重放：几乎不动 p=0.9、主要减少 p=0.5 的遗忘），不采用。
实现：`drift.update` 前 δ → τ·tanh(δ/τ)，默认关闭（trace 逐位一致）。
**在跑（2026-09-24 19:12 启动，预计 3~4 小时）**：`scratchpad/cap/`，
τ∈{5,10,30} × frozenlake p=0.4/0.6/0.8/0.9（60 trials）+ cliffwalking p=0.3/0.4（30 trials）。
对照（同设置，已有）：frozenlake 0.300/0.400/0.283/0.417；cliffwalking 0.667/0.567。
**采纳标准**：cliff p=0.3 下降 ≤1 SE 且 frozenlake 0.8/0.9 实质提升；否则改试标准化分数
(nll−H)/std(nll)。

**截断结果（2026-09-24 23:30）**：
| | fl 0.4 | fl 0.6 | fl 0.8 | fl 0.9 | cliff 0.3 | cliff 0.4 |
|---|---|---|---|---|---|---|
| 对照 | 0.300 | 0.400 | 0.283 | 0.417 | **0.667** | 0.567 |
| τ=5 | 0.283 | 0.417 | 0.383 | **0.683** | 0.433 | 0.667 |
| τ=10 | 0.400 | 0.350 | 0.383 | 0.567 | 0.533 | 0.433 |
| τ=30 | 0.400 | 0.317 | 0.350 | 0.533 | 0.400 | 0.667 |
frozenlake：τ 越小 p=0.9 越好，0.4/0.6 无明显损失（第一个不"挪短板"的候选）。
cliff p=0.3：三个 τ 全部低于对照（−0.13~−0.27）→ **按预设标准不通过**；但对 τ
不单调、SE≈0.09，可能有噪声 → 追加 cliff p=0.3 对照 vs τ=5 各 2 个 seed 复核。
按用户计划同时开始标准化分数 `--surprise-score z`（fl 4 点 + cliff 0.3/0.4）。
输出：`scratchpad/zscore/`。

**最终结果（2026-09-25 03:00）—— 两个方案都不采纳为默认**：
| | fl 0.4 | fl 0.6 | fl 0.8 | fl 0.9 | cliff 0.3 | cliff 0.4 |
|---|---|---|---|---|---|---|
| 原版 | 0.300 | 0.400 | 0.283 | 0.417 | 0.667 | 0.567 |
| 截断 τ=5 | 0.283 | 0.417 | 0.383 | **0.683** | 0.433 | 0.667 |
| 标准化 z | 0.350 | 0.317 | 0.383 | 0.583 | **0.267** | 0.700 |
cliff p=0.3 三 seed 复核（各 30 trials，seed 0/1/2）：原版 0.667/0.467/0.467（均值
0.534±0.053），截断 τ=5 0.433/0.367/0.300（均值 0.367±0.051）→ **差 −0.167，约
2.3 SE，每个 seed 都更低，是真实损失**（seed 1/2 原版恰好同为 0.467，已核对 trial 0
轨迹不同，是巧合）。z 在 cliff p=0.3 掉到 0.267，更差。
结论与线4 一致：凡是让"一次 slip 少忘一点"的办法，都在 frozenlake 温和点获益、在
cliffwalking 最剧烈的 p=0.3 付代价。开关保留（默认关闭），主表不启用。

### 线6（2026-09-25）：合作者的 clip surprise=2，全网格（3 环境 × p=0.3~0.9）

用户："我说了三个环境…你才跑了几个？而且，对方解决了：通过clip surprise score =2
可以解决。你这帮助也太少了。" → 我此前只测了 fl 4 点 + cliff 2 点、没测 bridge、没试
τ<5，是覆盖不足（已写入 feedback 记忆）。
新开关 `--surprise-clip c`（硬截断 min(δ,c)，默认关）。K=10、horizon 6、30 trials：

frozenlake
| p | cem_fir | cem_fir+clip2 | cem_ada | ada_mcts |
|---|---|---|---|---|
| 0.3 | 0.267 | 0.267 | 0.000 | 0.067 |
| 0.4 | 0.267 | 0.267 | 0.167 | 0.133 |
| 0.5 | 0.267 | 0.367 | 0.200 | 0.233 |
| 0.6 | 0.500 | 0.400 | 0.433 | 0.367 |
| 0.7 | 0.300 | 0.333 | 0.533 | 0.433 |
| 0.8 | 0.333 | 0.600 | 0.700 | 0.700 |
| 0.9 | 0.567 | 0.733 | 0.900 | 0.900 |

cliffwalking
| p | cem_fir | cem_fir+clip2 | cem_ada | ada_mcts |
|---|---|---|---|---|
| 0.3 | 0.667 | 0.133 | 0.100 | … |
| 0.4 | … | 0.567 | 0.233 | … |
| 0.5 | … | 0.867 | … | … |
| 0.6 | … | 0.967 | … | … |
...
| p | cem_fir | cem_fir+clip2 | cem_ada | ada_mcts |
|---|---|---|---|---|
| 0.3 | 0.200 | 0.133 | 0.133 | 0.033 |
| 0.4 | 0.200 | 0.167 | 0.167 | 0.067 |
| 0.5 | 0.333 | 0.367 | 0.400 | 0.233 |
| 0.6 | 0.433 | 0.600 | 0.633 | 0.300 |
| 0.7 | 0.733 | 0.833 | 0.800 | 0.500 |
| 0.8 | 0.867 | 0.867 | 0.867 | 0.533 |
| 0.9 | 1.000 | 1.000 | 1.000 | 0.867 |

（cliff 表中 "…" 的 p=0.4/0.5 可由同设置旧运行补：cem_fir 0.567/0.867，cem_ada
0.233/0.667，ada_mcts p=0.4 0.667；这些运行与本批重叠格逐位一致。）

读法：bridge 上 clip2 基本补齐 p=0.6/0.7 的缺口（0.433→0.600、0.733→0.833，与
cem_ada 持平）；frozenlake 上部分改善（p=0.8 0.333→0.600，p=0.9 0.567→0.733）但仍低于
cem_ada；**cliffwalking p=0.3 从 0.667 跌到 0.133（≈cem_ada 0.100）**，即在我们现行
代码（forget 后重置 δ̄）里，clip2 几乎关掉了遗忘，方法退化成 cem_ada。
合作者的代码是每步 forget 且不重置 δ̄ —— clip2 在那里是"每步 retain 至多减半、持续衰减"，
机制不同。**在跑**：`scratchpad/clip2c/`，`--surprise-clip 2 --k-forget 1
--no-forget-reset`（合作者配置）全网格，cem_fir only，2026-09-25 10:15 启动。
