...
**中间结果（24 个非平稳点已出 10 个，截至 2026-09-15 23:30）**

| config | p | 方法 | 旧 k=10 | 新 k=30 | 变化 |
|---|---|---|---|---|---|
| 1 | 0.3 | oracle | 0.700 | 0.833 | **+0.133** |
| 1 | 0.4 | oracle | 0.933 | 1.000 | +0.067 |
| 1 | 0.5 | ada-cem-cvar | 0.667 | 0.700 | +0.033 |
| 2 | 0.3 | oracle | 0.767 | 0.767 | 0 |
| 2 | 0.4 | oracle | 1.000 | 0.967 | −0.033 |
| 2 | 0.5 | ada-cem-cvar | 0.633 | 0.700 | +0.067 |
| 3 | 0.3 | oracle | 0.700 | 0.833 | **+0.133** |
| 3 | 0.3 | ada-cem-cvar | 0.533 | 0.533 | 0 |
| 3 | 0.4 | ada-cem-cvar | 0.767 | 0.767 | 0 |
| 3 | 0.5 | ada-cem-cvar | 0.900 | 0.933 | +0.033 |

**初步读法（尚未定论）**：真正的基线 `ada-cem-cvar` 对 k_models=30 几乎
无感（5 个点变化 0~+0.067，噪声量级），而 `oracle-cem` 在最难的 p=0.3 上
涨了 +0.133。若该趋势保持，公平修正后本文方法相对 `ada-cem-cvar` 的领先
基本不受影响，但 oracle 上限抬高。**决定性的还没出：`ada-cem-cvar` 在
config1/config2 的 p=0.3**（旧值只有 0.100/0.167，是本文方法领先最大的两
个点）。
注：`oracle_cem` 在 config1 与 config3 逐位相同是设计使然（它总是加载变化
后真实 p 的 checkpoint，与预训练 p 无关），两边都得 0.833，可作确定性自检。
