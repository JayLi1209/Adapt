...
- 日志：`$SCRATCH/aayl_repro_v2.log`；逐行实时结果在
  ...
  而会被缓冲，查进度要看这个文件）。
- ETA：低 p 点比预想慢（失败/徘徊的 episode 会跑满 100 步）。截至 13:07
  已出 5/7，父进程已跑 4h14m，**剩 p=0.4 / p=0.5，预计 14:00-15:30 出齐**。

**结果（截至 2026-09-16 13:07，5/7）——完整报告见
...
**两条结论（已可对 collaborator 交付）**：
1. 已完成的 4 个可比点，我们**每个都高于论文**，差 0.10~0.30——"复现不
   出来"的方向是偏好的一侧。
2. **论文 cliff 那一列几乎不可能是 γ=0.9999 算的**（硬证据三条：p=1.0 的
   `0.694 ± 0.00` 零方差在 ±1 终止奖励下不可能；论文 ±0.01~0.02 的误差条
   需要约 1600 次运行，而我们 n=30 实测 se=0.148；正文说有 per-step
   penalty 但 RATS 在 p=1.0 恰为 `0.000 ± 0.00` 又排除了它）。反推 γ≈0.996
   时 13 步最优路径给 0.958，恰好对上论文 MCTS-P_k 的 0.951。**在
   γ=0.9999 下无法重现 0.694~0.883 这个范围，这是口径问题不是实现问题。**
