...
**frozenlake 全表（K=10，horizon 6，60 trials；cem_ada 的 p=0.4/0.9 heuristic 列为 30 trials）**：
| p | 对照 | leaf | gate | ml | ml+gate | 三者叠加 | cem_ada | cem_ada(leaf) |
|---|---|---|---|---|---|---|---|---|
| 0.4 | 0.300 | 0.300 | 0.317 | 0.267 | 0.200 | 0.233 | 0.167 | 0.083 |
| 0.6 | 0.400 | 0.400 | 待 | 待 | 0.233 | **0.217** | 0.400 | 0.467 |
| 0.8 | **0.283** | 0.300 | 待 | 待 | 0.450 | 0.483 | 0.733 | 0.767 |
| 0.9 | **0.417** | 0.517 | 0.633 | 0.717 | 0.783 | **0.850** | 0.900 | 0.867 |

读法：对照是"温和变化比剧烈变化还差"（0.8 < 0.6）。三者叠加在 p=0.9 追平
（0.850 vs 0.867），p=0.8 砍掉一半差距，**但 p=0.6 从 0.400 掉到 0.217**
——不是净改进，是把短板挪了位置。leaf 单独：小幅、只在 0.8/0.9 起作用
（p=0.4/0.6 与对照逐位相同：只在"走/不走"边际决策处改变 argmax）。
ml-only / gate-only 在 0.6/0.8 正在跑，用来定位是谁伤了 p=0.6。

**cliffwalking（K=10，horizon 6，30 trials）**：p=0.9 全员 1.000（无差距）。
ml+gate：p=0.3 0.333 / p=0.4 **0.733**（对照 0.567，cem_ada 0.233）/ p=0.5
0.967 / p=0.9 1.000；对照 p=0.3/0.5 在跑（`p09w/cliff_ctrl_p35.log`）。

**⚠ 顺带发现（已在交付文档加更正说明）**：`--cem-horizon` 默认值是 3，主表
用 6。09-17 的两次运行漏了这个参数：36 格饱和点补跑（进了交付文档）和 K=10
retain factor 试验 → 实际都是 horizon 3。饱和点正在以 horizon 6 重跑
（`scratchpad/sat_h6/`），跑完替换交付文档 §1/§3 的更正段。K=10 retain
试验的内部对比仍有效（对照与变体同为 h=3），但不能称为"主表设置"。
...
**最终结果（24/24 全部完成，2026-09-16 05:20）**

`ada-cem-cvar` 全部 12 个点 + `oracle-cem` 变化最大的 6 个点：
...
| 1 | 0.6 | ada-cem-cvar | 0.800 | 0.800 | 0 | 0.967 |
| 2 | 0.6 | ada-cem-cvar | 0.900 | 0.900 | 0 | 1.000 |
| 3 | 0.6 | ada-cem-cvar | 1.000 | 1.000 | 0 | 1.000 |
...
（未列出的 `oracle` p=0.5/0.6 共 6 个点：config2/config3 的 p=0.5 分别
1.000/1.000、三组 p=0.6 全 1.000，除 config3 p=0.5 是 0.967→1.000 外全部
不变。）

**最终结论：公平对比后，本文方法相对 `ada-cem-cvar` 的领先完全站得住。**
- `ada-cem-cvar` 12 个点最大变化仅 +0.067，三个 p=0.3 点（领先最大处）
  分别是 −0.033 / 0 / 0；我方在 12 个非饱和点里 **赢 11、平 1**
  （config3 p=0.6 双方都 1.000）。
- 真正被抬高的是 `oracle-cem` 这个理论上限（p=0.3 +0.133、p=0.4 +0.067）。
  修正后它在**所有**点上都重新严格高于本文方法——此前 config1 p=0.4、
  config3 p=0.5 曾与我方持平，现在上限的合理性更干净了。
- 相对 `ada-mcts` 的 9 赢/3 平/1 微弱落后**完全不变**（它不用 CVaR-CEM，
  从头就不受此 bug 影响）。
- 机制解读（已写进报告 §4）：k_models 提升的是 CVaR 尾部估计的稳定性，
  **只有当底层模型本身已经正确时才兑现成收益**——oracle 直接在变化后真实
  p 上预训练，所以吃到全部红利；`ada-cem-cvar` 的瓶颈在它"先观望、攒够
  样本再切换"的 DPAS 机制本身，模型没跟上时把尾部估计做得再准也没用。
  即：我方的领先不是来自"给自己多调了 K"，而是 SFIR 让模型更快变正确、
  从而能把这个 K 用起来。
...
- 日志：`$SCRATCH/aayl_repro_v2.log`；逐行实时结果在
  ...
  而会被缓冲，查进度要看这个文件）。
- **✅ 已完成（2026-09-16 14:08，约 5h15m，`DONE.` 干净收尾、无报错）。**

**最终结果（7/7）——完整报告见
`doc_tool/ada_mcts_reproduction_2026-09-16.md`**

| p | 我们 goal rate | 我们回报 | 论文 ADA-MCTS | 差 |
|---|---|---|---|---|
| 0.4 | 0.800 | +0.597 | 0.778 | **−0.181** |
| 0.5 | 0.967 | +0.929 | 0.815 | **+0.114** |
| 0.6 | 0.967 | +0.930 | 0.830 | **+0.100** |
| 0.8 | 1.000 | +0.997 | 0.871 | **+0.126** |
| 0.9 | 1.000 | +0.996 | 0.883 | **+0.113** |
| 1.0 | 1.000 | +0.993 | 0.694 | **+0.299** |
| stationary(0.7) | 0.800 | +0.598 | 无对应行 | — |

**结论（可对 collaborator 交付）**：
1. **6 个可比点里 5 个高于论文、最难的 p=0.4 低于论文 0.181**（约 1.2 个
   我们自己的标准误，不足以断言"我们更差"）。差值符号在最易点(+0.30)和
   最难点(−0.18)之间翻转，这种不规则模式本身就不像"实现强弱"的差别。
   ⚠ 早前我曾说"每个点都高于论文"，那是只出了 5 个点时的说法，**已作废**。
2. **论文 cliff 那一列几乎不可能是 γ=0.9999 算的**（四条证据：p=1.0 的
   `0.694 ± 0.00` 零方差在 ±1 终止奖励下不可能；论文 ±0.01~0.02 的误差条
   需要约 1600 次运行，而我们 n=30 实测 se=0.148；正文说有 per-step
   penalty 但 RATS 在 p=1.0 恰为 `0.000 ± 0.00` 又排除了它；我们 p=1.0 的
   70 步轨迹在 γ=0.996 下重新计价得 0.755，已接近论文的 0.694，而在
   γ=0.9999 下必然是 0.99+）。**这是口径问题不是实现问题。**
3. **建议的决定性实验**：同一套实验用 γ≈0.996 重跑（只改一个参数，约 4
   小时），看 6 个点是否整体落到论文那一列附近。落上 → 证明差异纯粹来自
   折扣因子口径；落不上 → 才需要回头怀疑算法实现。**尚未启动，等用户定。**
...
1. **✅ 已完成（2026-09-16 05:20）**：公平性修正 24/24 跑完，两份报告
   （中/英）+ 自包含版的 ada-cem-cvar/oracle-cem 两行均已替换为新数字、
   "待修正"警告已全部撤掉，结论是我方领先完全保持（详见"实验结果"）。
   **collaborator 提到的 retain factor 试验也已完成（2026-09-16 17:47），
   结论：不采纳。** 试点（config1，candidates=256/trials=20，本项目验证过
   可用于相对比较的保真度；每个变体都带自己的对照）：

   | 变体 | p=0.3 | p=0.4 | p=0.9 |
   |---|---|---|---|
   | **对照（现行默认）** | **0.600** | **0.850** | 1.000 |
   | rho_floor=0.5 | 0.650 (+0.050) | 0.750 (−0.100) | 1.000 (0) |
   | retain_floor=0.3 | 0.400 (**−0.200**) | 0.650 (**−0.200**) | 1.000 (0) |
   | retain_floor=0.5 | 0.350 (**−0.250**) | 0.650 (**−0.200**) | 1.000 (0) |

   三条结论：
   (a) **p=0.9 已饱和**：四个变体全是 1.000。collaborator 说的"p=0.9 很差"
       是 k_models=10 时代的现象，k=30 之后症状本身就没了，**没有改善空间**。
   (b) **retain_floor 明确有害**：4 次独立测量（2 个下限值 × 2 个非饱和点）
       全部为负，且 p=0.3 上随下限变硬而单调变差（0.600→0.400→0.350）。
       方向与机制预期一致：保留越多旧信念，剧变时适应越慢。
   (c) **rho_floor 是噪声**：+0.050 / −0.100 / 0，方向不一致、幅度都在 1 个
       标准误内。另注：retain_floor=0.3 与 0.5 在 p=0.4 给出逐位相同的结果，
       说明起作用的是"有没有下限"而非下限取值。

   代码里两个开关（`--rho-floor` / `--retain-floor`）保留，默认值即现行行为
   （已逐位验证），**但不应在主表中启用**。

   **追加实验（2026-09-17 06:08 完成）：按用户澄清的原意，在
   `k_models=10`（谁都没调过规划器精度的原始设定）下重做主表**，
   512 candidates / 30 trials，config1+config2，p=0.3/0.4/0.5/0.9，
   4 变体各带自己的对照。8 个作业全部 `DONE`、零报错。
   下表为 goal rate，括号内为变体减对照：

   | 点 | ctrl | ada-mcts | rf03 | rf05 | rho05 |
   |---|---|---|---|---|---|
   | c1 p=0.3 | 0.433 | 0.367 | 0.433 (0) | 0.333 (−0.100) | 0.167 (**−0.266**) |
   | c1 p=0.4 | 0.833 | 0.667 | 0.667 (−0.166) | 0.633 (−0.200) | 0.533 (**−0.300**) |
   | c1 p=0.5 | 0.833 | 0.900 | 0.833 (0) | 0.833 (0) | 0.867 (+0.034) |
   | c2 p=0.3 | 0.433 | 0.400 | 0.433 (0) | 0.333 (−0.100) | 0.267 (−0.166) |
   | **c2 p=0.4** | **0.533** | **0.867** | **0.667 (+0.134)** | **0.667 (+0.134)** | 0.533 (0) |
   | c2 p=0.5 | 0.867 | 0.900 | 0.833 (−0.034) | 0.833 (−0.034) | 0.900 (+0.033) |
   | p=0.9（两组） | 1.000 | 1.000 | 1.000 (0) | 1.000 (0) | 1.000 (0) |
   | **6 个非饱和点净变化** | | | **−0.066** | **−0.300** | **−0.665** |

   **结论：不能。** (a) 唯一的实质短板 c2 p=0.4 补不上——最好的变体 0.667，
   离 ada-mcts 的 0.867 仍差 0.200；(b) 三个变体净收益全为负，且下限越硬
   越差；(c) retain_floor 在 c2 p=0.4 的 +0.134 被 c1 p=0.4 的 −0.166 抵消，
   同一 p 值两组场景符号相反，是噪声特征；(d) rho_floor 在关键点纹丝不动、
   却在两个 p=0.3 点退 0.166~0.266，**"clip rho"在 K=10 和 K=30 下均被证伪**。
   这再次印证：**c2 p=0.4 那个缺口，除 k_models=30 外没有别的旋钮补得上。**

   ⚠ **顺带发现，影响引用习惯**：现行代码**跑不出** 09-11 那批 K=10 旧数字
   （c1 p=0.4：现行 0.833 vs 旧 0.567；c2 p=0.5：现行 0.867 vs 旧 0.733；
   c1 p=0.3：现行 0.433 vs 旧 0.667），高低方向随机而非系统性偏移。时间线
   吻合：产出旧数字的提交 `5cf4aa3`（2026-09-11）早于把
   `torch.manual_seed(seed)` 引入 `_worker` 的 `b39ce69`（2026-09-12 11:18），
   随机数消耗路径变了。**这是"有机会"不是"已证实"**（未回滚代码验证）。
   实际影响：**09-11 那批 K=10 数字不能再当基线引用**；已发布的结果不受
   影响，因为主表（`a06cadf`，09-13）、ablation、公平性修正、ADA-MCTS 复现
   全部产生于 seed 接线之后。
   原始待办文字保留如下：用新数字替换
...
3. **✅ 已完成（2026-09-17 10:26）**：`k_models=30` 补跑到 p=0.7~1.0 的
   饱和点。起因是用户追问"那一列的 1.000 是不是 K=10 的数据"——核查后
   确认属实，而且连 `cem_fir` 那一列的来源也无法从文档考证（原始日志在
   `/tmp` 已丢失），于是索性把 3 个用 K 的方法全部重跑：
   3 方法 × 3 场景 × p=0.7/0.8/0.9/1.0 = **36 格，全部 1.000**，
   3 个作业均 `DONE`、零报错。交付文档中那段"⚠ 仍是 K=10 数据"的说明
   已删除，**现在全报告不存在任何 K 参数不一致的数字**。
   日志：`$SCRATCH/sat_k30_config{1,2,3}.log`。
...
- ~~**"公平对比之后，本文方法是不是真的最好？"**~~——**2026-09-16 已回答：
  是。** 24/24 点全部重跑完毕，`ada-cem-cvar` 对 `k_models=30` 基本无感
  （最大 +0.067），我方 12 个非饱和点赢 11 平 1；相对 `ada-mcts` 的
  9 赢/3 平/1 微弱落后不变；`oracle-cem` 作为上限被抬高且重新严格高于
  我方。详见"实验结果"与两份报告。**注意保留的限定**：ablation 显示
  SFIR 并非全面最优（单种子，待多种子复核），这是另一个独立问题。
